...
1. 先在 MySQL/PostgreSQL 中手动创建一个空数据库（如 `hmimg_db`）——向导会建所有表，但**不会建库**。
2. 启动前后端服务（见下文各部署方式）。`.env` 中数据库连接可留空——安装向导会收集并写回。
3. 打开站点，将自动跳转 `/install`。安装界面位于前端路由 `/install`，其调用的后端接口均在 `/api/install/*` 下（如 `GET /api/install/status`）。
4. 按向导操作：环境检查 → 数据库（测试连接、创建数据表；若已通过环境变量配置则自动跳过）→ 自定义管理员账号（密码至少 8 位）→ 站点设置（标题/默认语言/最大用户数/开放注册）。
5. 安装完成后向导自动锁定。旧版本部署（库中已有用户）升级时自动识别并补锁，无需重装。
...
### 准备数据库
先在 MySQL/PostgreSQL 中创建空数据库 `hmimg_db`（utf8mb4）——向导会建表，但不会建库。
...
### 准备数据库
在服务器上先创建空数据库 `hmimg_db`（utf8mb4）再启动后端——向导会建表，但不会建库。
